...
界面功能:
- 每个数据源下方实时显示速率/累计流量, 串口额外显示带宽占用百分比 (按波特率推算)
- 数据预览分 M/W 左右两个区域, 各自独立: 显示开关 (滑动, 转发中即时生效) / HEX/TXT / 暂停/继续/清空
- TCP服务端 / 组播 / 广播的对端列表, 可选择"主要对端" (TCP: 点对点; 不选: 发给全部收全部; 组播/广播: 只收主要对端)
- 文件发送: 进度条 + B/KB/MB 三滑块叠加限速 (另有"不限速"开关)
- 旁路记录: 自动创建时间戳文件夹 `2026-09-10_11-20-56`, 内含 `M.bin/M.txt/W.bin/W.txt` (BIN 为原始数据; TXT 可选周期性时间戳)
- 组播地址有效性检测 (224.0.0.0/4); 广播地址按网卡自动填充; 服务端端口占用检测; 串口占用标注
- 最下方事件日志, 可清空
...
# 文件限速发送 (b+1024*kb+1024*mb 字节每秒, 0/0/0=不限速) + 旁路记录
python -m transponder --nogui -m file-send:path=data.bin,b=500,kb=20,mb=0 \
                      -w tcp-server:port=9000 --log-dir logs --log-m --log-fmt bin

# 组播 (自动禁用组播回环) / 广播 (可指定广播地址与本地端口)
...
├── cli.py            # 命令行无UI模式
├── gui/              # PySide6 图形界面
│   ├── __init__.py   # run_gui 入口
│   ├── theme.py      # 现代深色主题 QSS + 滑动开关控件
│   ├── forms.py      # 各数据源类型的参数表单 (含对端列表/进度条逻辑所在面板)
│   ├── source_panel.py  # 单侧数据源面板 (类型/表单/打开/对端列表/统计)
│   └── main_window.py   # 主窗口 (布局/预览/记录配置/事件日志)
