...
- 提示消息为自动消失的浮动通知; 数据源"打开"按钮绿色 / "关闭"按钮红色
- 打开数据源后配置锁定变灰 (文件限速联动控件除外); 不限速时限速滑块灰掉, 接收模式下追加写入开关可用
- 每个数据源对应的数据预览区底部显示速率/累计流量, 停止转发后速率清零
...
- 文件发送: 进度条 + B/KB/MB 三滑块叠加限速 (滑块加长, 可动态调整), 智能单位显示; 单位标签默认显示
...
- 数字输入框"默认/自动"为真正空值的淡色占位提示, 清空输入自动回到占位; 自动端口/监听队列在打开后回填实际值
- 事件日志每行带统一宽度时间戳, 支持暂停/继续、清空、导出当前内容
