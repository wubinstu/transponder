...
class FileForm(QWidget):
    ...
    def __init__(self):
        ...
        self.rb_recv = QRadioButton("接收 → 写文件")
        ...
        # 两行模式控制: 每个 radio 与对应功能开关对齐, 仍由 QButtonGroup 互斥
        self.unlimited = SwitchToggle("不限速")
        self.unlimited.setChecked(True)
        self.append = SwitchToggle("追加写入")
        send_row = QHBoxLayout(); send_row.setSpacing(18)
        send_row.addWidget(self.rb_send); send_row.addWidget(self.unlimited); send_row.addStretch(1)
        recv_row = QHBoxLayout(); recv_row.setSpacing(18)
        recv_row.addWidget(self.rb_recv); recv_row.addWidget(self.append); recv_row.addStretch(1)
        lay.addLayout(send_row); lay.addLayout(recv_row)
...
class MulticastForm(QWidget):
    ...
    def __init__(self):
        ...
        def half(label: str, field: QWidget) -> QWidget:
            box = QWidget(); row = QHBoxLayout(box); row.setContentsMargins(0, 0, 0, 0); row.setSpacing(6)
